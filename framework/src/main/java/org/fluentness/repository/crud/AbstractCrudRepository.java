...
public abstract class AbstractCrudRepository<M extends Model> implements Repository<M> {
    ...
    public int insert(M model) {
        return persistence.persist("INSERT INTO " + tableName + "(" + getInto(model) + ") " + "VALUES (" + getValues(model) + ")");
    }

    public int update(M model) {
        return persistence.persist("UPDATE " + tableName + " SET " + getSet(model) + " WHERE id = " + model.getId());
    }
    ...
    protected String getInto(M model) {
        return Arrays.stream(model.getClass().getDeclaredFields()).map(Field::getName).collect(Collectors.joining(","));
    }

    protected String getValues(M model) {
        StringBuilder result = new StringBuilder();
        Field[] declaredFields = model.getClass().getDeclaredFields();
        try {
            for (int i = 0; i < declaredFields.length; i++) {
                result.append(i == 0 ? "" : ", ").append(declaredFields[i].get(model));
            }
    ...
    protected String getSet(M model) {
        try {
            StringBuilder result = new StringBuilder();
            Field[] declaredFields = model.getClass().getDeclaredFields();
            for (int i = 0; i < declaredFields.length; i++) {
                ...
                result.append(i == 0 ? "" : ", ").append(field.getName()).append(" = ").append(field.get(model));
            }
            return result.toString();
        } catch (IllegalAccessException e) {
            e.printStackTrace();
